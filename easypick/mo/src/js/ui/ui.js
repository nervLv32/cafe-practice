...
(function ($, core, ui, undefined) {
  ...
  ui.SELECT = function () {
    ...
    this.events = {
      ...
      _setVariable: function () {
        var w = [$$.anchor.outerWidth(true), $$.group.outerWidth(true)];
        w.sort(function (a, b) {
          return b - a;
        });
  ...
  ui.INPUTFILE = function () {
    ...
    this.events = {
      _init: function () {
        $$.container.on("change", "input[type='file']", $.proxy(this._uploadFile, this));
        $$.list.on("click", ".btn-remove", $.proxy(this._removeFile, this));
        // $$.container.on('update', this._update); // 2021-07-29 EZP-3633 주석처리
      },
      _uploadFile: function (e) {
        var el = e.target,
          files,
        ...
        var $li = $("<li class='ui-sortable-helper'></li>");
        $$.list.append($li);
        ...
        for (var i = 0; i < filesLength; i++) {
          (function (index, inst) {
            ...
            // 2021-06-30 EZP-3360
            $item.append("<span class='preview-img-link'>" + fileName + " 미리보기</span><button type='button' class='btn-remove'>삭제</button>");
            $li.append($item);
        ...
        if (filesLength == isRegisterFileName.length) {
          $li.remove();
        }
      ...
      _removeFile: function (e) {
        ...
        if (data.type == "only") {
          $$.container.removeClass("regist").find("input[type='file']").val("").empty();
      ...
      _update: function () {
        var n = 0;
        data.fileList.forEach(function (acc) {
          n += acc.length;
        });
  ...
  ui.SLIDE = function () {
    ...
    this.events = {
      ...
      calc: {
        SlideWidth: function () {
          return $$.view.width() / opts.columnGroup;
        },
        CurrentIndex: function () {
          return !opts.loop ? opts.currentIndex : opts.currentIndex + 1;
        },
        Indexing: function () {},
  ...
  ui.DIRECTION = function () {
    ...
    this.events = {
      _init: function () {
        var Event = core.browser.isTouch ? ["touchstart", "touchmove", "touchend"] : ["mousedown", "mousemove", "mouseup"];
  ...
  ui.Tooltip = function () {
    ...
    this.events = {
      ...
      _hide: function () {
        ...
        if (data.animation) {
          TweenMax.to($$.tooltip, data.animation.duration / 1000, {
            css: data.animation.to,
            onComplete: function () {
  ...
  ui.POPUP = {
    ...
    bindEvent: function () {
      ...
      // $body.on('touchstart.updown', '.ui-popup.active.updown > .content', this.handleTouchContent);

      $body.on("POPUP_OPEN", ".ui-popup", this.openTrigger);
      $body.on("POPUP_CLOSE", ".ui-popup", this.closeTrigger);
    },
    // handleTouchContent: function(e){
    // 	var $cnt = $(this),
    // 		$popup = $cnt.parent(),
    // 		dur = $cnt.css('transition-duration'),
    // 		startY = e.touches[0].screenY,
    // 		distY;

    // 	$cnt.off('touchmove.updown').on('touchmove.updown', function(e){
    ...
    // 	$cnt.off('touchend.updown').on('touchend.updown', function(){
    // 		if(distY > 0){
    // 			$cnt.css('transition-duration', dur);
    // 			$popup.trigger('POPUP_CLOSE');
    // 		}
    // 	});

    // 	$cnt.off('transitionend.updown').on('transitionend.updown', function(){
    // 		$cnt.css('transform', 'translateY(0px)');
    // 	});
    // },
    detectClickDim: function (e) {
      // var target = e.target,
      // 	isDim = target.className.indexOf('ui-popup') > -1;
      //
      // if(isDim){
      ...
      // 2021-04-13 EZP-2649
      if (isDim && !$target.hasClass("dim-evt")) {
        $target.trigger("POPUP_CLOSE");

        if ($target.data("close_reload") == "T") {
          location.reload();
        }
      }
    },
    ...
    openTrigger: function (e, fn) {
      ...
      /* 2022-09-30 EZP-7096 modal open시 focus 제거 */
				$('input:focus').blur();
    },
    ...
    openPopup: function ($popup) {
      $popup.addClass("active").trigger("COMPLETE_POPUP_OPEN");
    ...
    closePopup: function () {
      var isIframe = self !== top,
        ...
        // $popup = isIframe ? $('.ui-popup', parent.document) : $(this).closest('.ui-popup'),
        $parentPopup = isIframe ? $(window.frameElement).closest($popup) : $(this).closest($popup);

      $parentPopup.removeClass("active").trigger("COMPLETE_POPUP_CLOSE");

      // $parentPopup.children('.content').css('height', 'auto'); 풀팝업 스크롤 사라짐
    ...
    scrollDisable: function ($popup) {
      ...
      core.$body
        .css({
          overflow: "hidden",
          position: "fixed",
          top: (-ui.POPUP.scrollTop + Number($("html").css("margin-top").replace("px", ""))) + "px", //2022-05-12 수정 / 2022-05-19 괄호처리
          width: "100%",
        })
  ...
  ui.TAB = {
    ...
    detectAnchorActive: function () {
      ...
      $anchor.eq(near[0].index).addClass("active").siblings().removeClass("active");

      $content.eq(near[0].index).addClass("active").siblings().removeClass("active");
    },
    /* ui.STICKY 사용

...
(function($, core, ui, undefined) {
  ...
  ui.SELECT = function() {
    ...
    this.events = {
      _init: function() {
        if (opts.disabled || $$.container.hasClass("disabled")) {
          return;
        }
        this._set();
      ...
      _outputSelect: function(evt) {
        ...
        $$.item.removeClass("active");
        for (var i in opts.selectItems) {
          if ($$.item.eq(opts.selectItems[i].idx).hasClass("all")) {
            continue;
          }
  ...
  ui.FILEINPUT = function() {
    ...
    this.events = {
      ...
      _uploadFile: function(e) {
        ...
        /* 2020-02-14 추가 sjkim */
        var sDelBtn = '<button type=\'button\'>삭제</button>';
        if ($$.container.find("input[type='file']").hasClass('not-addclear') === true) {
          sDelBtn = '';
        }

        var fileName = (opts.FileReader) ? el.files[0].name : fileValue.replace(/^.+\\/, "");
        if (opts.type !== "multifle") {
          $$.list.html("<div><span>" + fileName + "</span>" + sDelBtn + "</div>");
          opts.fileList[0] = fileValue;
        } else {
          if (String(opts.fileList).indexOf(fileValue) > -1) {
            return this._cancleFile();
          }
          $$.list.append("<div><span>" + fileName + "</span>" + sDelBtn + "</div>");
  ...
  ui.ACCORDION = function() {
    var $$ = core.Selector(arguments[0], {
      item: ">.acc-item",
      head: ">.acc-item >.acc-head",
      cont: ">.acc-item >.acc-cont"
    });
    var opts = core.DataSet($$._dataSet, {
      sync: false,
    ...
    this.events = {
      ...
      _set: function() {
        $$.cont.hide();
        if (opts.activeIdx == 'all') {
          $$.item.addClass("active");
          $$.cont.show();
        } else if (opts.activeIdx > -1) {
          $$.item.eq(opts.activeIdx).children(".acc-cont").show();
        }
      },
      _accSelect: function(e) {
        ...
        if ($root.hasClass("active")) {
          ...
          $root.children(".acc-cont").slideUp(opts.duration, function() {
            dev.observer.notify('SYNC_LEFT');
          });
        } else {
          if (opts.sync) {
            $$.item.removeClass("active").children(".acc-cont").slideUp(opts.duration, function() {
              dev.observer.notify('SYNC_LEFT');
            });
          }
          $root.addClass("active");
          $root.children(".acc-cont").slideDown(opts.duration, function() {
            dev.observer.notify('SYNC_LEFT');
          });
  ...
  ui.POPUP = function() {
    ...
    this.events = {
      ...
      open: {
        _check: function() {
          ...
          switch (opts.popup.type) {
            case "layer":
              ...
              break;
            case "link":
              this.open._link.call(this);
              break;
            case "iframe":
              this.open._iframe.call(this);
              break;
            case "window":
              this.open._window();
              // 2021-05-24 EZP-2889
        ...
        _link: function() {
          $$.content.on('load',opts.popup.url, $.proxy(function() {
            this.open._layer.call(this);
          }, this));
        },
        _window: function() {
      ...
      _close: function() {
        ...
        //레이어 팝업 갯수를 구해서, 팝업갯수가 0 이상이면  밑에 로직 타지 않도록 처리함
        var modals = $('.ui-popup.on');
        if (modals.length > 0) {
            $('body').css('overflow','hidden');
        } else{
            // 2021-06-30 EZP-3357
      ...
      detect: {
        _opts: function() {
          ...
          if ($$.navigation.length > 0) {
            this.set._navigation();
          }
          if (opts.pager == "list") {
            this.create._pagerList();
          }
          if (opts.pager == "number") {
            this.create._pagerNumber();
          }
          if (opts.loop) {
            this.create._cloneItem.call(this);
          }
        },
        _resize: function() {
        ...
        $$.target.each(function(i) {
          ...
          */
          if (complete >= data.length) {
            core.observer.off("SCROLL", events._detectPosition);
          }
        });
      }
    }
  ...
  ui.LEFT_MENU = function() {
    ...
    this.events = {
      ...
      bindEvent: function() {
        core.observer.on('LOAD SYNC_LEFT', this.syncHeight);
      },
      syncHeight: function() {
        $container.css('height', 'auto');

        var left_height = $left.prop('scrollHeight'),
          cont_height = $container.outerHeight(),
          sync_height = left_height > cont_height ? left_height + 100 : cont_height;

        $container.css('height', sync_height);
      }
    }
  }

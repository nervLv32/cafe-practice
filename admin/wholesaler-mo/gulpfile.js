...
gulp.task('css', () => {
  return new Promise(resolve => {
    gulp
      .src([
        PATH.ASSETS.SCSS + '/*.css',
        PATH.ASSETS.SCSS + '/*.css.map',
      ])
...
gulp.task('css2', () => {
  return new Promise(resolve => {
    gulp
      .src([
        PATH.ASSETS.SCSS2 + '/*.scss',
        PATH.ASSETS.SCSS2 + '/**/*.scss',
      ])
      .pipe(gulp.dest(DEST_PATH.ASSETS.CSS2))
    resolve()
  })
})
...
gulp.task('img', () => {
  return new Promise(resolve => {
    gulp
      ...
      .pipe(gulp.dest(DEST_PATH.ASSETS.IMG))
    resolve()
  })
})
...
gulp.task('watch', () => {
  return new Promise(resolve => {
    ...
    const css_watcher = gulp.watch(PATH.ASSETS.SCSS + '/**/*.css', { ignoreInitial: false }, gulp.series(['css']))   // css 폴더 내의 모든 파일 감시
    file_management(css_watcher, PATH.ASSETS.SCSS, DEST_PATH.ASSETS.CSS)                             // src > css 폴더 내의 삭제되는 파일 감시하여 dist에서 삭제

    const css2_watcher = gulp.watch(PATH.ASSETS.SCSS2 + '/**/*.scss', { ignoreInitial: false }, gulp.series(['css2']))   // css 폴더 내의 모든 파일 감시
    file_management(css2_watcher, PATH.ASSETS.SCSS2, DEST_PATH.ASSETS.CSS2)                             // src > css 폴더 내의 삭제되는 파일 감시하여 dist에서 삭제

    const img_watcher = gulp.watch(PATH.ASSETS.IMG + '/**/*', gulp.series(['img']))          // IMG 폴더 내의 모든 파일 감시
    file_management(img_watcher, PATH.ASSETS.IMG, DEST_PATH.ASSETS.IMG)                      // src > img 폴더 내의 삭제되는 파일 감시하여 dist에서 삭제

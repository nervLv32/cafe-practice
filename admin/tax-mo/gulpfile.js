...
const config = {
  src: './src',
  dist: './dist',
  guide_common: '/guide-files/common',
  guide_guide: '/guide-files/guide'
}
...
gulp.task('img1', () => {
  return new Promise(resolve => {
    gulp
      ...
      .pipe(gulp.dest(DEST_PATH.ASSETS.IMG1))
    resolve()
  })
})
...
gulp.task('watch', () => {
  return new Promise(resolve => {
    ...
    const img1_watcher = gulp.watch(PATH.ASSETS.IMG1 + '/**/*', gulp.series(['img1']))          // IMG1 폴더 내의 모든 파일 감시
    file_management(img1_watcher, PATH.ASSETS.IMG1, DEST_PATH.ASSETS.IMG1)                      // src > img1 폴더 내의 삭제되는 파일 감시하여 dist에서 삭제

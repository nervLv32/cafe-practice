...
const PATH = {
  ...
  ASSETS: {
    ...
    VIDEO: config.src + config.assets + '/video',
    JS: config.src + config.assets + '/js',
    LIB: config.src + config.assets + '/lib',
    DATA: config.src + config.data,
...
const DEST_PATH = {
  ...
  ASSETS: {
    ...
    VIDEO: config.dist + '/video',
    JS: config.dist + '/js/ui',
    LIB: config.dist + '/js/ui/lib'
  }
}
...
gulp.task('njk', () => {
  return new Promise(resolve => {
    gulp
      ...
      .pipe(replace('../assets/video/', '../../video/'))
...
gulp.task('favicon', () => {
  return new Promise(resolve => {
    gulp.src(config.src + '/html/favicon.ico')
      .pipe(gulp.dest(config.dist))
    resolve()
  })
})
...
gulp.task('watch', () => {
  return new Promise(resolve => {
    ...
    const video_watcher = gulp.watch(PATH.ASSETS.VIDEO + '/**/*', gulp.series(['video']))          // images 폴더 내의 모든 파일 감시
    file_management(video_watcher, PATH.ASSETS.VIDEO, DEST_PATH.ASSETS.VIDEO)                      // src > images 폴더 내의 삭제되는 파일 감시하여 dist에서 삭제
    ...
    const js1_watcher = gulp.watch(PATH.HTML + '/**/*.js', gulp.series(['js']))                   // js 폴더 내의 모든 파일 감시
    file_management(js1_watcher, PATH.HTML, DEST_PATH.HTML)                                  // src > js 폴더 내의 삭제되는 파일 감시하여 dist에서 삭제

    const lib_watcher = gulp.watch(PATH.ASSETS.LIB + '/**/*', gulp.series(['lib']))                   // lib 폴더 내의 모든 파일 감시
    file_management(lib_watcher, PATH.ASSETS.LIB, DEST_PATH.ASSETS.LIB)                               // src > lib 폴더 내의 삭제되는 파일 감시하여 dist에서 삭제
